...
export const LibraryAdmin = (props) => {
  const [modifyMethod, setModifyMethod] = useState('ModifyMethod');
  return (
    <Window
...
const SearchAndDisplay = (props) => {
  ...
  const [modifyTarget, setModifyTarget] = useState(0);
  const {
    can_db_request,
    search_categories = [],
    book_id,
  ...
  const books = flow([
    // Implemented a fix here since old tgui and new tgui have differing arguments for map and sortBy
    (pages) => map<Book, DisplayBook>(pages, (book, i) => ({
      ...book,
      // Generate a unique id
      key: i,
    })),
    (pages) => sortBy<DisplayBook>(pages, (book) => book.key),
  ])(pages);
  return (
    <Section>
        ...
        <Stack.Item>
          <Stack>
            <Stack.Item>
              <Input
                value={book_id}
                placeholder={!book_id ? 'ID' : String(book_id) || undefined}
                width="70px"
                onChange={(e, value) =>
                  act('set_search_id', {
                    id: value,

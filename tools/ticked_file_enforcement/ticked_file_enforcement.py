...
# horrible but i'm not touching this any more than i have to
prefixed_excluded_files = [x for x in excluded_files]
if subdirectories:
    prefixed_excluded_files = [(scannable_directory + x).replace('/', '\\') for x in excluded_files]
...
for code_file in scannable_files:
    ...
    if not included:
        ...
        if(dm_path in prefixed_excluded_files):
            continue

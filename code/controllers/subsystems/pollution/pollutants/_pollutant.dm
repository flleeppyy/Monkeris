...
/datum/pollution/proc/scrub_amount(amount_to_scrub, update_active = TRUE)
	if(amount_to_scrub >= total_amount || iswall(my_turf) || QDELING(my_turf))
		qdel(src)
		return
	for(var/type in pollutants)
...
/datum/pollution/proc/process_cell()
	...
	for(var/turf/open/open_turf as anything in my_turf.GetAtmosAdjacentTurfs())
		if(iswall(open_turf) || QDELING(open_turf))
			continue
	...
	for(var/turf/open/open_turf as anything in sharing_turfs)
		if(iswall(open_turf) || QDELING(open_turf) || QDELETED(open_turf.pollution))
			continue
...
/turf/proc/update_adjacent_pollutants()
	for(var/turf/open/open_turf as anything in GetAtmosAdjacentTurfs())
		if(iswall(open_turf) || QDELING(open_turf) || QDELETED(open_turf.pollution))
			continue
		SET_ACTIVE_POLLUTION(open_turf.pollution)
...
/turf/wall/pollute_turf(pollution_type, amount, cap)
	return

/turf/wall/pollute_turf_list(list/pollutions, cap)
	return

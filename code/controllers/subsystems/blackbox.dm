...
/datum/controller/subsystem/blackbox/proc/FinalFeedback()
	...
	for (var/datum/ticket/ticket in SStickets.allTickets)
...
/datum/controller/subsystem/blackbox/proc/LogBroadcast(freq)
	...
	switch(freq)
		...
		if(DTH_FREQ)
			record_feedback("tally", "radio_usage", 1, "specialops")
		if(SYND_FREQ)
			record_feedback("tally", "radio_usage", 1, "mercenary")
		if(YARR_FREQ)
			record_feedback("tally", "radio_usage", 1, "pirate")
		if(SUP_FREQ)
			record_feedback("tally", "radio_usage", 1, "supply")
		if(NT_FREQ)
			record_feedback("tally", "radio_usage", 1, "ntvoice")
		if(SRV_FREQ)
			record_feedback("tally", "radio_usage", 1, "service")
		if(AI_FREQ)
			record_feedback("tally", "radio_usage", 1, "ai private")
		if(MED_I_FREQ)
			record_feedback("tally", "radio_usage", 1, "medicali")
		if(SEC_I_FREQ)
			record_feedback("tally", "radio_usage", 1, "securityi")
		else
...
/datum/controller/subsystem/blackbox/proc/ReportDeath(mob/living/L)
	...
	var/datum/db_query/query_report_death = SSdbcore.NewQuery({"
		INSERT INTO [format_table_name("death")] (pod, x_coord, y_coord, z_coord, mapname, server_ip, server_port, round_id, tod, job, special, name, byondkey, laname, lakey, bruteloss, fireloss, brainloss, oxyloss, toxloss, cloneloss, last_words, suicide)
		VALUES (:pod, :x_coord, :y_coord, :z_coord, :map, INET_ATON(:internet_address), :port, :round_id, :time, :job, :special, :name, :key, :laname, :lakey, :brute, :fire, :brain, :oxy, :tox, :clone, :last_words, :suicide)
	"}, list(
		"name" = L.real_name,
		"key" = L.ckey,
		"job" = L.mind.assigned_role,
		// TODO: figure out what "special" role actually is
		"special" = null,
		...
		"fire" = L.getFireLoss(),
		"brain" = L.getBrainLoss(), //get_organ_loss returns null without a brain but a value is required for this column
		"oxy" = L.getOxyLoss(),
		"tox" = L.getToxLoss(),

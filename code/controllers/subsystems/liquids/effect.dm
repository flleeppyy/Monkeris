...
/obj/effect/abstract/liquid_turf/update_overlays()
	...
	if(number != 0)
		add_overlay(mutable_appearance('icons/effects/liquid_overlays.dmi', "stage[number]_bottom", plane = calculate_plane(z, GAME_PLANE), layer = ABOVE_MOB_LAYER))
		add_overlay(mutable_appearance('icons/effects/liquid_overlays.dmi', "stage[number]_top", plane = calculate_plane(z, GAME_PLANE), layer = BELOW_MOB_LAYER))
...
/obj/effect/abstract/liquid_turf/Initialize(mapload, datum/liquid_group/group_to_add)
	...
	GLOB.entered_event.register(my_turf, src, /obj/effect/abstract/liquid_turf/proc/movable_entered)
...
/obj/effect/abstract/liquid_turf/proc/movable_entered(datum/source, atom/movable/AM)
	...
	if(liquid_group.group_overlay_state >= LIQUID_STATE_ANKLES)
		...
		if(isliving(AM))
			...
			liquid_group.expose_atom(stepped_human, 1, CHEM_TOUCH)

...
/datum/liquid_group/New(height, obj/effect/abstract/liquid_turf/created_liquid)
	color = "#[random_string(3, GLOB.hex_characters)]"
...
/datum/liquid_group/proc/merge_group(datum/liquid_group/otherg)
	...
	add_reagents(reagent_list = created_reagent_list, chem_temp = 300)
...
/datum/liquid_group/proc/process_turf_disperse()
	...
	if(reagents_per_turf < 5)
		...
		for(var/turf/edge_turf in cached_edge_turfs)
			...
			for(var/turf/adjacent in edge_turf.GetAtmosAdjacentTurfs())
		...
		while(turfs_to_remove > 0 && length(cached_edge_turfs))
			...
			if(!QDELETED(best_candidate.liquids))
				...
				for(var/dir in GLOB.cardinal)
					...
					if(iswall(open_turf) || QDELETED(open_turf.liquids))
						continue
...
/datum/liquid_group/proc/transfer_to_atom(obj/effect/abstract/liquid_turf/remover, amount, atom/transfer_target, transfer_method = CHEM_INGEST)
...
/datum/liquid_group/proc/move_liquid_group(obj/effect/abstract/liquid_turf/member)
	...
	for(var/datum/reagent/reagent_type in reagents.reagent_list)
		member.liquid_group.reagents.add_reagent(reagent_type, remove_amount, safety = TRUE)
...
/datum/liquid_group/proc/add_reagent(obj/effect/abstract/liquid_turf/member, datum/reagent/reagent, amount, temperature)
	reagents.add_reagent(reagent, amount, safety = TRUE)
...
/datum/liquid_group/proc/transfer_reagents_to_secondary_group(obj/effect/abstract/liquid_turf/member, obj/effect/abstract/liquid_turf/transfer)
	...
	for(var/datum/reagent/reagent_type in reagents.reagent_list)
		transfer.liquid_group.reagents.add_reagent(reagent_type.type, remove_amount, safety = TRUE)
		remove_specific(amount = remove_amount, reagent_type = reagent_type)
...
/datum/liquid_group/proc/transfer_specific_reagents(datum/reagents/secondary_reagent, amount, list/reagents_to_check, obj/effect/abstract/liquid_turf/remover, merge = FALSE)
	...
	for(var/datum/reagent/reagent_type in reagents.reagent_list)
		if(!(reagent_type.type in reagents_to_check))
			continue
		secondary_reagent.add_reagent(reagent_type.type, reagent_type.volume * precent, safety = TRUE)
		if(remover)
...
/datum/liquid_group/proc/process_spread(turf/member)
	...
	for(var/turf/open/adjacent_turf in cached_fire_spreads[member])
		if(!prob((group_burn_power) * modifier))
			continue
...
/datum/liquid_group/proc/process_cached_edges()
	for(var/turf/cached_turf in cached_edge_turfs)
		for(var/direction in cached_edge_turfs[cached_turf])
			...
			if(iswall(directional_turf))
				continue
			if(!(directional_turf in cached_turf.GetAtmosAdjacentTurfs())) //i hate that this is needed
				continue
...
/datum/liquid_group/proc/return_connected_liquids(obj/effect/abstract/liquid_turf/source, adjacent_checks = 0)
	...
	while(length(queued_liquids))
		...
		for(var/turf/adjacent_turf in queued_turf.GetAtmosAdjacentTurfs())
			if(QDELETED(adjacent_turf.liquids) || !members[adjacent_turf])
				continue
			if(!(adjacent_turf in queued_turf.GetAtmosAdjacentTurfs())) //i hate that this is needed
				continue
...
/datum/liquid_group/proc/try_split(turf/source, return_list = FALSE)
	...
	for(var/turf/adjacent_turf in head_turf.GetAtmosAdjacentTurfs())
...
/datum/liquid_group/proc/try_bulk_split(list/input_turfs)
	...
	for(var/turf/listed_input in input_turfs)
		for(var/turf/cardinal in listed_input.GetAtmosAdjacentTurfs())
...
/datum/liquid_group/proc/expose_members_turf(obj/effect/abstract/liquid_turf/member)
	...
	for(var/atom/movable/target_atom in members_turf)
		turf_reagents.touch(target_atom)

/datum/liquid_group/proc/expose_atom(atom/target, modifier = 0, method)
	if(!turf_reagents)
		return
	turf_reagents.touch(target)

/datum/liquid_group/proc/spread_liquid(turf/new_turf, turf/source_turf)
	if(iswall(new_turf) || !source_turf.GetAtmosAdjacentTurfs())
		return
	if(!(new_turf in source_turf.GetAtmosAdjacentTurfs())) //i hate that this is needed
		return

	if(new_turf.is_transparent)
		var/turf/Z_turf_below = GetBelow(new_turf)
		if(!Z_turf_below)
			return
	...
	if(QDELETED(new_turf.liquids) && !new_turf.is_transparent && source_turf.turf_height == new_turf.turf_height) // no space turfs, or oceans turfs, also don't attempt to spread onto a turf that already has liquids wastes processing time
...
/datum/liquid_group/proc/water_rush(turf/new_turf, turf/source_turf)
	...
	for(var/atom/movable/target_atom in new_turf)
		if(!target_atom.anchored && !target_atom.pulledby && !isobserver(target_atom))
			reagents.trans_to(target_atom, CHEM_TOUCH, (reagents_per_turf * 0.5))
			...
			if(isliving(target_atom) && prob(60))
				...
				target_living.Paralyse(6 SECONDS)

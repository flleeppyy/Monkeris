...
/datum/storyteller/proc/storyteller_panel()
	...
	data += "<b><a href='byond://?src=\ref[src];panel=1'>\[UPDATE\]</a></b>"
	data += "<table><tr><td>"
	data += "[src.name] (<A href='byond://?src=\ref[src];c_mode=1'>Change</A>)"
	...
	data += "Heads: [heads] "
	if(debug_mode)
		data += "<a href='byond://?src=\ref[src];edit_heads=1'>\[EDIT\]</a>"
	data += "<br>Ironhammer: [sec] "
	if(debug_mode)
		data += "<a href='byond://?src=\ref[src];edit_sec=1'>\[EDIT\]</a>"
	data += "<br>Technomancers: [eng] "
	if(debug_mode)
		data += "<a href='byond://?src=\ref[src];edit_eng=1'>\[EDIT\]</a>"
	data += "<br>Medical: [med] "
	if(debug_mode)
		data += "<a href='byond://?src=\ref[src];edit_med=1'>\[EDIT\]</a>"
	data += "<br>Science: [sci] "
	if(debug_mode)
		data += "<a href='byond://?src=\ref[src];edit_sci=1'>\[EDIT\]</a>"
	...
	data += "<hr>"
	data += "<b><a href='byond://?src=\ref[src];force_spawn=1'>\[FORCE ROLE SPAWN\]</a></b>"
	data += "<hr>"
	data += "<B>Evacuation</B>"
	if (!evacuation_controller.is_idle())
		data += "<a href='byond://?src=\ref[src];call_shuttle=1'>Call Evacuation</a><br>"
	else
	...
	if(length(GLOB.current_antags))
		for(var/datum/antagonist/A in GLOB.current_antags)
			...
			data += "<li>[A.role_text] - [A.owner?(A.owner.name):"no owner"] ([act])<a href='byond://?src=\ref[A];panel=1'>\[EDIT\]</a></li>"
	else
	...
	for(var/list/L in list(event_pool_mundane, event_pool_moderate, event_pool_major, event_pool_roleset))
		...
		for (var/datum/storyevent/S in L)
			...
			data += "<a href='byond://?src=\ref[src];event=[S.id];ev_debug=1'>\[VV\]</a>"
			data += "<b><a href='byond://?src=\ref[src];event=[S.id];ev_spawn=1;severity=[severity]'>\[FORCE\]</a></b></li>"
			data += "</li>"
		data += "</ul>"
		data += "-------------------------<BR>"

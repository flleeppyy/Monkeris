...
/atom/vv_get_dropdown()
	...
	VV_DROPDOWN_OPTION(VV_HK_SPASM, "Spasm")
...
/atom/vv_do_topic(list/href_list)
	...
	if(href_list[VV_HK_SPIN_ANIMATION] && check_rights(R_FUN))
	...
	if(href_list[VV_HK_SPASM] && check_rights(R_FUN))
		var/loops = input(usr, "How many loops?", "Spasm Animation") as null|num
		if (isnull(loops))
			return

		if (loops > VV_MAX_SPASM_LOOPS)
			to_chat(usr, span_warning("[loops] is too many loops! Must be less than [VV_MAX_SPASM_LOOPS]"))
			return
		spasm_animation(loops)
		return

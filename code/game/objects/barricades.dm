...
/obj/structure/barrier/four_way/attackby(obj/item/I, mob/user)
	if(istype(I, /obj/item/device/assembly/signaler))
		...
		if((signaler.code == code) && (signaler.frequency == frequency)) // Linked signaler is used, break the link
			...
			signaler.audible_message("[icon2html(signaler, hearers(src))] *beep*")
		else if(!frequency || !code) // There is no link, establish one
			update_wifi_password(signaler.code, signaler.frequency)
			signaler.audible_message("[icon2html(signaler, hearers(src))] *beep* *beep*")
		else // Access denied
			signaler.audible_message("[icon2html(signaler, hearers(src))] *beep* *beep* *beep*") // Silly and cryptic, yet authentic signaler feedback
		return

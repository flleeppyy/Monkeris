...
/obj/item/device/techno_tribalism/attack_self()
	if(world.time >= (last_produce + cooldown))
		if(items_count >= max_count)
			...
			visible_message("[icon2html(src, hearers(src))] The [src] beeps, \"The [src] is not full enough to produce.\".")
	else
		visible_message("[icon2html(src, hearers(src))] The [src] beeps, \"The [src] needs time to cooldown.\".")

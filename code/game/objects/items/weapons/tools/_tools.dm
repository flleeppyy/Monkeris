...
/obj/item/tool/proc/eyecheck(mob/user)
	...
	if(ishuman(user))
		...
		if(safety <= FLASH_PROTECTION_REDUCED)
		...
		else if(safety <= FLASH_PROTECTION_NONE)
			to_chat(H, span_warning("Your eyes burn."))
			E.take_damage(rand(8, 12), BURN)
			if(E.damage > 10)
				E.take_damage(rand(4, 12))
				H.eye_hazy += rand(1,3)
				H.eye_blurry += rand(4,8)
		else if(safety <= FLASH_PROTECTION_MINOR)

...
/proc/client_preference_stats_for_usr(mob/user = usr)
	...
	if(!client_preference_stats_)
		...
		for(var/datum/client_preference/client_pref in get_client_preferences())
			client_preference_stats_[client_pref.description] = new /atom/movable/stat_client_preference(null, client_pref)

	for(var/client_pref_description in client_preference_stats_)
		var/atom/movable/stat_client_preference/scp = client_preference_stats_[client_pref_description]
...
/client/verb/toggle_preference_verb(client_pref_name in client_preference_stats_for_usr())
	...
	var/atom/movable/stat_client_preference/scp = client_stats[client_pref_name]
	if(istype(scp))
		scp.Click()

/atom/movable/stat_client_preference
	simulated = FALSE
	var/datum/client_preference/client_preference

/atom/movable/stat_client_preference/New(loc, preference)
	client_preference = preference
	update_name(usr)
	..()

/atom/movable/stat_client_preference/Destroy()
	client_preference = null
	. = ..()

/atom/movable/stat_client_preference/Click()
	if(!usr.client)
		return
...
/atom/movable/stat_client_preference/proc/update_name(mob/user)
	if(!user || !user.client)
		return

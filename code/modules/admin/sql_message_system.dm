/proc/create_message(type, target_key, admin_ckey, text, timestamp, server, secret, logged = 1, browse, expiry, note_severity)
	...
	var/list/parameters = list(
		...
		"playtime_mins" = SSjob.GetTotalPlaytimeMinutesCkey(target_ckey)
	)
	if(timestamp)
		parameters["timestamp"] = timestamp
	// TODO: Port DB playtimes
	...
	var/datum/db_query/query_create_message = SSdbcore.NewQuery({"
		INSERT INTO [format_table_name("messages")] (type, targetckey, adminckey, text, timestamp, server, server_ip, server_port, round_id, secret, expire_timestamp, severity, playtime)
		VALUES (:type, :target_ckey, :admin_ckey, :text, [timestamp? ":timestamp" : "Now()"], :server, INET_ATON(:internet_address), :port, :round_id, :secret, :expiry, :note_severity, :playtime_mins)
	"}, parameters)
...
/proc/browse_messages(type, target_ckey, index, linkless = FALSE, filter, agegate = FALSE)
	...
	if(target_ckey)
		...
		while(query_get_messages.NextRow())
			...
			// TODO: Change back when DB playtimes are ported
			// data += "<b>[timestamp] | [server] | Round [round_id] | [admin_key][secret ? " | <i>- Secret</i>" : ""] | [get_exp_format(text2num(playtime))] Living Playtime"
			data += "<b>[timestamp] | [server] | Round [round_id] | [admin_key][secret ? " | <i>- Secret</i>" : ""] | [get_exp_format(text2num(playtime))] Total Playtime"
			if(expire_timestamp)
	...
	while(query_get_message_output.NextRow())
		...
		switch(type)
			if("message")
				...
				output += "<br><font color='[COLOR_RED]'>[text] <A href='byond://?_src_=holder;[HrefToken()];messageread=[message_id]'>(Click here to verify you have read this message)</A></font><br>"
			if("note")
			...
			if("watchlist entry")
				// previously message_high_admins

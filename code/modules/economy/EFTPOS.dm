...
/obj/item/device/eftpos/attackby(obj/item/O as obj, user as mob)
	...
	if(I)
		if(linked_account)
			...
			to_chat(usr, "[ma2html(src, usr)][span_warning("Unable to connect to linked account.")]")
			...
			if(linked_account.is_valid())
				if(transaction_locked && !transaction_paid)
					if(transaction_amount <= E.worth)
						...
						to_chat(usr, "[ma2html(src, usr)][span_warning("\The [O] doesn't have that much money!")]")
			else
				to_chat(usr, "[ma2html(src, usr)][span_warning("Connected account has been suspended.")]")
		else
		...
		switch(href_list["choice"])
			if("change_code")
				...
				if(attempt_code == access_code)
					...
					to_chat(usr, "[ma2html(src, usr)][span_warning("Incorrect code entered.")]")
			if("change_id")
			...
			if("link_account")
				...
				if(linked_account)
					if(!linked_account.is_valid())
						linked_account = null
						to_chat(usr, "[ma2html(src, usr)][span_warning("Account has been suspended.")]")
				else
					to_chat(usr, "[ma2html(src, usr)][span_warning("Account not found.")]")
			if("trans_purpose")
			...
			if("toggle_lock")
				if(transaction_locked)
					...
					to_chat(usr, "[ma2html(src, usr)][span_warning("No account connected to send transactions to.")]")
			if("scan_card")
				if(linked_account)
					...
					to_chat(usr, "[ma2html(src, usr)][span_warning("Unable to link accounts.")]")
			if("reset")
				...
				if (istype(I, /obj/item/card))
					...
					if((access_cent_captain in C.access) || (access_hop in C.access) || (access_captain in C.access))
						access_code = 0
						to_chat(usr, "[ma2html(src, usr)][span_info("Access code reset to 0.")]")
...
/obj/item/device/eftpos/proc/scan_card(obj/item/card/I, obj/item/ID_container)
	...
	if(transaction_locked && !transaction_paid)
		...
		if(!D)
			to_chat(usr, "[ma2html(src, usr)][span_warning("Unable to access account. Check security settings and try again.")]")
			return

		if(!D.is_valid())
			to_chat(usr, "[ma2html(src, usr)][span_warning("Your account has been suspended.")]")
			return
		...
		if(!(transaction_amount <= D.money))
			...
			return
		playsound(src, 'sound/machines/chime.ogg', 50, 1)
		src.visible_message("<img src='\ref[src]'> \The [src] chimes.", push_apperance = src)
		transaction_paid = 1
...
/obj/item/device/eftpos/emag_act(remaining_charges, mob/user, emag_source)
	if (!transaction_locked)
		return
	if(transaction_paid)
		to_chat(usr, "[ma2html(src, usr)][span_info("You stealthily swipe \the [emag_source] through \the [src].")]")
		transaction_locked = 0
		transaction_paid = 0
	else
		usr.visible_message(span_info("\The [usr] swipes a card through \the [src]."))

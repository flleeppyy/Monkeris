...
/datum/nano_module/program/computer_ntnetmonitor/nano_ui_interact(mob/user, ui_key = "main", datum/nanoui/ui = null, force_open = NANOUI_FOCUS, datum/nano_topic_state/state = GLOB.default_state)
	if(!ntnet_global)
		return
	...
	if(!user.stat_check(STAT_COG, STAT_LEVEL_BASIC))
		var/datum/extension/fake_data/fake_data = get_or_create_extension(src, /datum/extension/fake_data, /datum/extension/fake_data)

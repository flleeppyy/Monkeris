//TODO: Allow shoving books in peoples faces and make them read the book.
/obj/item/book
	name = "book"
...
/obj/item/book/attack_self(mob/user)
	...
	if(carved)
		if(store)
			to_chat(user, span_notice("[store] falls out of [book_data.title]!"))
			store.loc = get_turf(src.loc)
			store = null
			return
		else
			to_chat(user, span_notice("The pages of [book_data.title] have been cut out!"))
			return
...
/obj/item/book/attackby(obj/item/attacking_item, mob/living/user, params)
	// at some point... port burn_paper_product_attackby_check maybe?
	if(burning)
		return

	if (carved)
		if(!store)
			if(attacking_item.w_class < ITEM_SIZE_NORMAL)
				user.drop_item()
				attacking_item.loc = src
				store = attacking_item
				to_chat(user, span_notice("You put [attacking_item] in [book_data.title]."))
				return
			else
				to_chat(user, span_notice("[attacking_item] won't fit in [book_data.title]."))
				return
		else
			to_chat(user, span_notice("There's already something in [book_data.title]!"))
			return
...
/obj/item/book/proc/try_carve(obj/item/carving_item, mob/living/user, params)
	...
	if(!(user.a_intent == I_HURT))
		return FALSE
	if(!(QUALITY_CUTTING in carving_item.tool_qualities))
		return FALSE

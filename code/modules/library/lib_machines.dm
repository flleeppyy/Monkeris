...
/obj/machinery/computer/libraryconsole/ui_act(action, list/params, datum/tgui/ui, datum/ui_state/state)
	...
	switch(action)
		...
		if("search")
			if(!prevent_db_spam())
				say_quote("Database cables refreshing. Please wait a moment.")
				return
			INVOKE_ASYNC(src, PROC_REF(update_db_info))
			return TRUE
		if("switch_page")
...
/obj/machinery/computer/libraryconsole/bookmanagement/ui_data(mob/user)
	...
	data["display_lore"] = (emagged && can_spawn_lore)
...
/obj/machinery/computer/libraryconsole/bookmanagement/proc/print_forbidden_lore(mob/user)
	...
	var/obj/item/tool/knife/ritual/knife = new(get_turf(src))
	knife.sanity_damage += 1
	if (ishuman(user))
		var/mob/living/carbon/human/human = user
		human.sanity.onShock(10) // The fucking horrors.
	...
	user.visible_message(span_warning("[user] stares at the blank screen for a few moments, their expression frozen in fear. When they finally awaken from it, they look a lot older."), vision_distance = 2)
...
/obj/machinery/computer/libraryconsole/bookmanagement/proc/upload_from_scanner(upload_category)
	var/obj/machinery/libraryscanner/scan = get_scanner()
	// All say_quote() calls here should eventually be replaced with a future refactor that adds atom/movable/say_quote()
	if(!scan)
	...
	if(!book.title)
		balloon_alert_to_viewers("No title detected. Aborting")
		return
	if(!book.author)
		balloon_alert_to_viewers("No author detected. Aborting")
		return
	if(!book.content)
		balloon_alert_to_viewers("No content detected. Aborting")
		return
...
/obj/machinery/bookbinder/proc/prebind_book(mob/user, obj/item/paper/draw_from)
	...
	if(!scanned_name)
		...
		balloon_alert_to_viewers("No ID detected...")
		say_quote("No ID detected. Please scan your ID if you would like to be credited for this book. Otherwise please enter your paper again.")
		return

	if(!user.unEquip(draw_from, src))
		return

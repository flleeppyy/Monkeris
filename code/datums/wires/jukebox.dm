...
/datum/wires/jukebox/UpdatePulsed(wire)
	...
	switch(wire)
		if(WIRE_MAIN_POWER1)
			holder.visible_message(span_notice("The power light flickers."))
			A.shock(usr, 90)
		if(WIRE_REVERSE)
			holder.visible_message(span_notice("The data light blinks ominously."))
		if(WIRE_SPEEDUP)
			holder.visible_message(span_notice("The speakers squeaks."))
		if(WIRE_SPEEDDOWN)
			holder.visible_message(span_notice("The speakers rumble."))
		if(WIRE_START)
			A.StartPlaying()
		if(WIRE_STOP)
			A.StopPlaying()

#define SMES_WIRE_RCON 		(1 << 0)	// Remote control (AI and consoles), cut to disable
#define SMES_WIRE_INPUT 	(1 << 1)	// Input wire, cut to disable input, pulse to disable for 60s
#define SMES_WIRE_OUTPUT 	(1 << 3)	// Output wire, cut to disable output, pulse to disable for 60s
#define SMES_WIRE_GROUNDING (1 << 4)	// Cut to quickly discharge causing sparks, pulse to only create few sparks
#define SMES_WIRE_FAILSAFES (1 << 5)	// Cut to disable failsafes, mend to reenable

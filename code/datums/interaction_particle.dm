...
/mob/proc/animate_interact(atom/target, state)
	...
	particle.set_plane(ABOVE_ALL_MOB_LAYER)
...
/mob/living/carbon/get_hand_pixels()
	var/obj/item/organ/external/hand = has_active_hand()
	if(!hand)
		return null
	else
		return hand.get_offset(dir)

/obj/item/organ/external/proc/get_offset(direction)
	. = null

	if (body_part & ARM_RIGHT)
		switch(direction)
			if(NORTH)
				return list(6,-3)
			if(SOUTH)
